...
'''
这个算法需要 对每个输入的测试数据计算他与所有的训练集数据之间的距离 
（可以是 曼哈顿距离 L1， 欧式距离 L2）， 然后挑选出其中距离最小的k个值作为 选民， 
并根据他们的党派进行投票， 这是一种典型的少数服从多数的方法
'''
...
class KNearestNeighbor(object):
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      closest_y_label = self.y_train[temp]
      #########################################################################
      # TODO:                                                                 #
    ...
    for i in range(num_test):
      temp = np.argsort(dists[i])[0:k]
      y_pred[i] = np.bincount(self.y_train[temp]).argmax()
